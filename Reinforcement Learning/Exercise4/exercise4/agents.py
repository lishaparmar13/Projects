...
class DDPG(Agent):
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        states, actions, n_states, rewards, done = (torch.as_tensor(batch[i], dtype=torch.float32) for i in range(5))

        # Compute Q-values for current states
        cur_q_val = self.critic(torch.cat((states, actions), dim=1))

        # Compute Q-values for next states using the critic target and actor target networks
        with torch.no_grad():
            n_actions = self.actor_target(n_states)
            n_q_values = self.critic_target(torch.cat((n_states, n_actions), dim=1))
            target_q_values = rewards + (self.gamma * n_q_values * (1 - done))

        # Compute critic loss
        q_loss = mse_loss(cur_q_val, target_q_values.detach())
