    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(*args, **kwargs):
            ...
            epsilon = self.epsilon_start - ((self.epsilon_start - self.epsilon_min) / (self.exploration_fraction * max_timestep)) * timestep
            return max(self.epsilon_min, self.epsilon)

        def epsilon_exponential_decay(*args, **kwargs):
            ### PUT YOUR CODE HERE ###
            "self.epsilon = max( self.epsilon_min, self.epsilon_start * self.epsilon_decay ** (timestep / max_timestep))"
            if timestep >= self.exploration_fraction * max_timestep:
                return self.epsilon_min

            epsilon_decay = self.epsilon_exponential_decay_factor ** (1 / max_timestep)
            self.epsilon_start *= epsilon_decay
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        """
        ### PUT YOUR CODE HERE ###
        if explore and np.random.rand() < self.epsilon: # explorative action
            action = self.action_space.sample()
        else: # exploitive action
            obs = Tensor(obs)
            q_values = self.critics_net(obs).detach().cpu().numpy().squeeze()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        expected_q_values = (next_q_values * self.gamma) * (1 - done) + rewards
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        with torch.no_grad():
            ...
            action_probs = self.policy(obs_tensor).squeeze(0)

        if explore: action = Categorical(action_probs).sample().item()
        else: action = action_probs.argmax().item()
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        """
        ### PUT YOUR CODE HERE ###
        # Compute the discounted returns G_t for each time step t
        G = torch.zeros(1, dtype=torch.float)
        returns = []
        for r in rewards[::-1]:
            G = self.gamma * G + r
            returns.insert(0, G)
