    ...
    def act(self, obs: int) -> int:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        if random.random() < self.epsilon:
            ...
            q_values = [self.q_table.get((obs, i), 0) for i in range(self.n_acts)]
            q_value_max = max(q_values)
            q_actions_max = [i for i, q_value in enumerate(q_values) if q_value == q_value_max]
            return random.choice(q_actions_max)
...
class QLearningAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        n_q_values = [self.q_table[(n_obs, i)] for i in range(self.n_acts)]
        max_n_q_values = max(n_q_values) if n_q_values else 0
        td_error = reward + self.gamma * max_n_q_values - self.q_table[(obs, action)]
...
class MonteCarloAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        pairs = list(zip(obses,actions))

        for t in range(T-1,-1,-1):
            G = self.gamma * G + rewards[t]

            if (obses[t],actions[t]) not in pairs[:t]:
                #updated_Q = self.q_table[(obses[t], actions[t])] + G

                if (obses[t], actions[t]) not in self.sa_counts: 
                    self.sa_counts[(obses[t], actions[t])] = 0

                self.sa_counts[(obses[t], actions[t])] += 1
                updated_Q = self.q_table[(obses[t], actions[t])] + (1 / self.sa_counts[(obses[t], actions[t])]) * (G - self.q_table[(obses[t], actions[t])])
                self.q_table[(obses[t], actions[t])] = updated_Q
                updated_values[(obses[t], actions[t])] = updated_Q
